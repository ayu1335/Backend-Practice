...
import { Usermodel, Todomodel } from './models/db';
import {z} from 'zod';
...
app.post("/signup",async (req, res) => {
    const userSchema = z.object({
        username:z.string().min(2).max(3),
        password:z.string().min(5).max(13)
    });
    const parseduserSchema=userSchema.safeParse(req.body);
    if(!parseduserSchema.success){
        return res.status(403).json({
            msg:"invaild",
            error:parseduserSchema.error
        })
    }
    const { username, password } = parseduserSchema.data;
...
app.post("/signin", (req, res) => {
    ...
    if (user) {
        const token = sign({ username, userId: user.userId }, JWT_SECRET_KEY, { expiresIn: "1d" });
